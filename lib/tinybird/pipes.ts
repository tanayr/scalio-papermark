...
export async function getTotalAvgPageDuration(p:Filter & {documentId:string}) {
 const totals=await prisma.pageEvent.groupBy({by:['versionNumber','variant','pageNumber','viewId'],where:{documentId:p.documentId,...filters(p)},_sum:{duration:true}});
 const groups=new Map<string,{versionNumber:number;variant:"DESKTOP"|"MOBILE";pageNumber:string;sum:number;count:number}>();
 for(const r of totals){const key=`${r.versionNumber}:${r.variant}:${r.pageNumber}`;const g=groups.get(key)||{versionNumber:r.versionNumber,variant:r.variant,pageNumber:r.pageNumber,sum:0,count:0};g.sum+=r._sum.duration||0;g.count++;groups.set(key,g);}
 return {data:Array.from(groups.values()).map(g=>({versionNumber:g.versionNumber,variant:g.variant,pageNumber:g.pageNumber,avg_duration:g.sum/g.count})).sort((a,b)=>a.versionNumber-b.versionNumber || Number(a.pageNumber)-Number(b.pageNumber))};
}
