...
export function emailAllowed(link:Link,email:string) {
 ...
 return !link.isArchived && (!link.expiresAt || link.expiresAt>new Date()) && !link.denyList.some(matches) && (!link.allowList.length || link.allowList.some(matches));
}
export async function viewerSession(req:NextApiRequest,link:Link) {
 ...
 return session && session.linkId===link.id && session.expiresAt>new Date() && emailAllowed(link,session.email) ? session : null;
}
export async function authorizeViewer(req:NextApiRequest,res:NextApiResponse,link:Link) {
 const existing=await viewerSession(req,link); if(existing) return existing.email;
 const email=normalizedEmail(req.body.email || req.body.verifiedEmail);
 if(!email.includes('@') || !emailAllowed(link,email)) {res.status(403).json({message:'This email does not have access to this link.'});return null;}
